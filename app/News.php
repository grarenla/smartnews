...
class News extends Model
{
    ...
    {
        try {
            $list = News::join('categories', 'categories.id', 'news.category_id')->select('categories.id as category_id', 'categories.name as category_name', 'news.*')->orderBy('news.id')->paginate(20);
            return $list;
        } catch (Exception $e) {
            return $e->getMessage();
        }
